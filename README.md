...
├─ next.config.ts            # Next.js 配置（安全头 / viewTransition / bundle analyzer）
...
| 层级      | 工具                     | 数量               | 覆盖范围                                                              |
| --------- | ------------------------ | ------------------ | --------------------------------------------------------------------- |
| 单元/集成 | Vitest + Testing Library | 505 用例 / 59 文件 | lib 数据层 / 组件交互 / 页面渲染                                      |
...
- 站点配置唯一源：`src/lib/site.ts`；内容路径与分页常量在 `src/lib/content-dirs.ts`
