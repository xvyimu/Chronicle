...
- 文章 published 字段
- 站点 URL 配置（从 `src/lib/constants.ts` 读取，无需单独维护元信息）
...
### site.config.json（已合并入 constants.ts）

此文件曾在 `src/lib/constants.ts` 之外承载冗余的站点名称/描述/作者信息。

当前状态：**已删除**。站点配置已全部收敛到 `src/lib/constants.ts` 作为单一可信源。不再需要单独维护。
...
- 构建期生成部分派生资产，例如 RSS
- 通过类型和校验降低内容格式错误风险
- Manifest PWA 元数据由 `src/app/manifest.ts` 从 `constants.ts` 动态生成
...
站点配置已全部收敛到 `src/lib/constants.ts` 作为单一可信源。`site.config.json`（已删除）、原 RSS 脚本中的 inline 常量（已迁移为从 `constants.ts` 导入）、`public/manifest.json`（已替换为 `app/manifest.ts`）全部汇入。
