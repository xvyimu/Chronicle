...
```
globals.css          ← 入口：@import Tailwind + 5 个自定义 CSS 文件
├── styles/tokens.css       ← 设计令牌（CSS 变量）+ reset + 主题
├── styles/layout.css       ← 布局组件（Header / Footer / ReadingProgress）
├── styles/components.css   ← 通用组件（Section / Card / Hero / Btn / Blog）
├── styles/prose.css        ← MDX 文章排版样式
└── styles/responsive.css   ← 响应式断点覆盖
```
...
| `layout.css` | 页面骨架组件 | `.header`、`.footer`、`.reading-progress` |
| `components.css` | 可复用 UI 组件 | `.section`、`.card`、`.btn`、`.hero`、`.blog__item` |
| `prose.css` | MDX 渲染的文章排版 | `.prose h2`、`.prose code`、`.code-toolbar` |
| `responsive.css` | 媒体查询覆盖 | `@media (max-width: 768px)` |

**规则**：新组件的 CSS 放入 `components.css`。如果 `components.css` 超过 500 行，考虑按组件拆分为独立文件（如 `_hero.css`、`_card.css`）并用 `@import` 引入。
