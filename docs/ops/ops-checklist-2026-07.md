# Chronicle · 运营清单 · 2026-07（W3 刷新）

> 组合包：`portfolio-arch-upgrade-2026h2` · 波次 **W3**  
> 刷新日：2026-07-23 · 分支 `xvyimu/w3-ch-claude`  
...
| ID           | 项                                                              | 状态                              | Owner         | 入口                                                 |
| ------------ | --------------------------------------------------------------- | --------------------------------- | ------------- | ---------------------------------------------------- |
| OPS-GSC      | Google Search Console：域名 `incca.ccwu.cc` + DNS TXT + sitemap | **`blocked_auth`**（需人账号）    | 用户          | ops-deferred §3                                      |
| OPS-Bing     | Bing Webmaster 从 GSC 导入                                      | **`blocked_auth`**（接在 GSC 后） | 用户          | ops-deferred §4                                      |
| OPS-SEO-LIVE | 生产 sitemap/robots/home 公开面                                 | 工程门禁已就绪；发版后跑 live     | CI + 本地     | `pnpm check:ops-readiness -- --live`                 |
| OPS-RUM      | Speed Insights 六页 p75 回填                                    | **`pending`** 样本 / 只读 token   | 用户（可选）  | performance-baseline · ops-deferred §5               |
| OPS-AUDIT    | `pnpm audit --audit-level=high`                                 | 每波复跑 · 目标 high=0            | CI + 本地     | npmjs registry                                       |
| OPS-SRI      | SRI 单元 + offline 门闩                                         | 每波复跑                          | CI + 本地     | `test:sri` · `check:sri-smoke`                       |
| OPS-PROD     | 生产内容 + 安全头冒烟                                           | 有网时                            | 本地 / deploy | `check:production-content`                           |
| OPS-PERF     | 性能预算表                                                      | **W3 已落盘**                     | 文档          | [`perf-budget-2026-07.md`](./perf-budget-2026-07.md) |
