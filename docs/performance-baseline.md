...
> 所有断言通过（CI success）。`/blog/nextjs-app-router` 的 perf=0.83 略低于 0.85 阈值、CLS=0.13 略超 0.1，但 CI 取 2 次中位数通过。2026-07-05 的 CI 回归确认主要来源是固定背景层 `.site-backdrop__mesh` 的 transform 动画；该动画已改为静态装饰。TBT 全部 0–3 ms，远低于 300 ms 阈值，印证 SSG 静态站的响应性优势。Speed Insights p75 见下表，需生产流量后填充。
...
- Check viewport-fixed decorative layers for transform animations, especially `.site-backdrop__stage` and `.site-backdrop__mesh`.
