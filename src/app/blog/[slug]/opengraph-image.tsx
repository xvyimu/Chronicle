...
export default async function OGImage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!post) {
    return new ImageResponse(
      (
        <div style={{ display: 'flex', flexDirection: 'column', width: '100%', height: '100%', background: '#0f0f1a', color: '#e4e4f0', padding: '60px', justifyContent: 'center' }}>
    ...
    (
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <div style={{ fontSize: 28, fontWeight: 700, color: '#818cf8' }}>{SITE_CONFIG.name}</div>
