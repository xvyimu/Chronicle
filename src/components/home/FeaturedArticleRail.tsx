...
export default function FeaturedArticleRail({ posts }: FeaturedArticleRailProps) {
  ...
  return (
    <section className="section home-article-rail" aria-labelledby="home-article-rail-title">
      <div className="section__inner">
        <div className="section__head">
          <div>
            <span className="section__eyebrow">Featured Articles</span>
            <h2 id="home-article-rail-title" className="section__title">最新文章</h2>
            <p className="section__subtitle">把最近整理过的实践笔记放成一条可横向浏览的轨道。</p>
          </div>
          ...
          {posts.map((post, index) => (
            <article key={post.slug} className="home-article-rail__card">
              <Link href={`/blog/${post.slug}`} className="home-article-rail__link">
                <span className="home-article-rail__number">{String(index + 1).padStart(2, '0')}</span>
